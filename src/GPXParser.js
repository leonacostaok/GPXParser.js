...
gpxParser.prototype.parse = function (gpxstring) {
    ...
    for (let idx in trks){
	    ...
	    for (let idxIn in trkpts){
            ...
            // extras GARMIN
            let extensions = keepThis.queryDirectSelector(trkpt, "extensions");
            if (extensions) {
                const heartRate = extensions.getElementsByTagNameNS(
                    "http://www.garmin.com/xmlschemas/TrackPointExtension/v1",
                    "hr",
                );
                extras.hr = heartRate.length > 0 ? heartRate[0].innerHTML : undefined

                const cadence = extensions.getElementsByTagNameNS(
                    "http://www.garmin.com/xmlschemas/TrackPointExtension/v1",
                    "cad",
                );
                extras.cad = cadence.length > 0 ? cadence[0].innerHTML : undefined
            }
            pt.extras = extras
